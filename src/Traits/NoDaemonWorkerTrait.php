...
namespace Queueworker\NoDaemon\Traits;

trait NoDaemonWorkerTrait
{
    /**
     ...
     * @return void
     */
    protected function runNoDaemon($connection, $queue)
    {
    ...
    public function processJobs($connectionName, $queue)
    {
        foreach (explode(',', $queue) as $queue) {
            while ($this->shouldRunNextJob() && ! is_null($job = $this->getNextJob($connectionName, $queue))) {
                $this->worker->runNoDaemonJob($job, $connectionName, parent::gatherWorkerOptions());
    ...
    {
        return $this->worker->getNextNoDaemonJob(
