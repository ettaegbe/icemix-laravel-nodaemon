...
namespace Queueworker\NoDaemon\Console;

use Illuminate\Contracts\Cache\Repository as Cache;
use Illuminate\Queue\Console\WorkCommand as BaseWorkCommand;
use Illuminate\Queue\Worker;
use Queueworker\NoDaemon\Traits\NoDaemonWorkerTrait;

class WorkCommand extends BaseWorkCommand
{
    use NoDaemonWorkerTrait;
    ...
    public function __construct(Worker $worker, Cache $cache)
    {
        ...
        $this->signature .= '{--nodaemon : Run the worker without a daemon}
    ...
    {
        if ($this->option('nodaemon')) {
            $this->worker->setCache($this->laravel['cache']->driver());

            return $this->runNoDaemon($connection, $queue);
        }
